...
def loss_fn(features, mode, params):
    ...
    if params.use_mask:
        ...
        if params.foreground_dilation > 0:
            ...
            mask = tf.nn.max_pool(
                mask, [1, pool_size, pool_size, 1], [1] * 4, 'SAME')
    ...
    endpoints['residual_translation'] = tf.split(
        residual_translation, 2, axis=0)
    ...
    disp0, disp1 = tf.split(
        aggregator.output_endpoints['disparity'], 2, axis=0)
...
def input_fn(params):
    ...
    if params.image_preprocessing.data_augmentation:
        if params.learn_intrinsics.per_video:
            raise (
                'Data augemnation together with learn_intrinsics.per_video is not '
                'yet supported.')

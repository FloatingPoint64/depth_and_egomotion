...
class LossAggregator(six.with_metaclass(abc.ABCMeta, object)):
    ...
    def __init__(
        ...
        self._losses = {k: tf.convert_to_tensor(
            0.0) for k in self._default_weights}
...
class DepthMotionFieldLossAggregator(LossAggregator):
    ...
    def _calculate(self):
        ...
        if 'groundtruth_depth' in self._endpoints:
            gt_depth_stack_ = tf.concat(
                self._endpoints['groundtruth_depth'], axis=0)
            gt_depth_pyramid = _get_pyramid(
        ...
        for s in reversed(range(self._params.num_scales)):
            ...
            self._losses['depth_smoothing'] += (scale_w *
                                                regularizers.joint_bilateral_smoothing(disp /
                                                                                       mean_disp, rgb_stack))
            ...
            flipped_rotation = tf.concat(
                self._endpoints['rotation'][::-1], axis=0)
            ...
            self._losses['motion_drift'] += scale_w * \
                regularizers.sqrt_sparsity(normalized_trans)
            self._losses['depth_consistency'] += (
                scale_w * loss_endpoints['depth_error'])
            self._losses['rgb_consistency'] += scale_w * \
                loss_endpoints['rgb_error']
            self._losses['ssim'] += scale_w * \
                0.5 * loss_endpoints['ssim_error']

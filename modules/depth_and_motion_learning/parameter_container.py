...
def parse_object(object_or_json_or_file, allow_dict=False, allow_list=False):
    ...
    if not allow_dict and not allow_list:
        raise ValueError(
            'At least one of allow_dict or allow_list must be True.')
    ...
    if isinstance(object_or_json_or_file, six.string_types):
        ...
        try:
            ...
            # None by json.loads).
            try:
                object_or_json_or_file = ast.literal_eval(
                    object_or_json_or_file)
                # Looking for a literal dict, no need for isinstance.
                if isinstance(object_or_json_or_file,
                ...
                elif isinstance(object_or_json_or_file, list):  # pylint: disable=unidiomatic-typecheck
                    if not allow_list:
                        raise ValueError(
            ...
            try:
                ...
                message = (
                    'Unable to parse override parameters either as a literal '
                    ...
                    'Override parameters:\n%s.\n' %
                    (file_error_str,
                     str(literal_json_parsing_error),
                        str(python_parsing_error),
                        object_or_json_or_file))
...
class ParameterContainer(object):
    ...
    def override(self,
        ...
        if strictness_depth is not None:
            if not is_strict:
                raise ValueError(
                    'strictness_depth can only be specified when is_strict'
                    ' is true')
            if strictness_depth <= 0:
                raise ValueError(
                    'strictness_depth must be positive, not %d' %
                    strictness_depth)
    ...
    def _override(self,
        ...
        for k, v in six.iteritems(params_dict):
            if not isinstance(k, six.string_types):
                raise ValueError(
                    'The keys in the dictionary must be strings, \'%s\' '
                    'encountered (type: %s).' %
                    (str(k), type(k)))
            k, indices = _get_key_and_indices(k)
            if indices is not None and not isinstance(self.__dict__[k], list):
                raise ValueError(
                    'Only parameters with list values can use '
                    'indices (key=%s, index0=%d)' %
                    (k, indices[0]))
            ...
            if isinstance(v, dict):
                if hasattr(self, k) and isinstance(self.__dict__[k],
                    ...
                    self.__dict__[k]._override(
                        v, is_custom, is_strict, strictness_depth, allow_custom_override)
                    # pylint: enable=protected-access
                else:
                    ...
                    if hasattr(self, k):
                        if isinstance(
                            ...
                            # No known handling strategy for the paramater
                            # configuration.
                            raise TypeError(
                                'Parameter {} cannot be overridden with {}'.format(
                                    k, v))
                    elif is_strict:
                        # Only existing keys can be overridden if is_strict.
                        raise AttributeError(
                            'Parameter not recognized: %s' % k)
                    else:
                        # Unknown parameter, create empty container at k and
                        # override it.
                ...
                if is_strict or k in self.__dict__:
                    self.__setattr__(
                        k, v, allow_custom_override, indices=indices)
                else:
    ...
    def _raise_if_custom(self, key):
        if hasattr(self, '_custom_params') and key in self._custom_params:
            raise AttributeError(
                'Parameter %s is immutable because it has a custom value.' %
                key)

    def _raise_if_locked(self):
        if hasattr(self, '_lock') and self._lock:
            raise AttributeError(
                'Cannot set parameter: the ParameterContainer '
                'is locked.')
    ...
    def __setattr__(
        ...
        if key in self.__dict__ and isinstance(self.__dict__[key], StringEnum):
            ...
            if indices is not None:
                self.__dict__[key] = self._set_nested_attribute(
                    self.__dict__[key], indices, value)
            else:
